...
def population_plot(case_name, dynamic, fnames, part):
    tasks = []
    for fname in fnames:
	...
	if not ave_tasks[1] == 0:
        ...
        if part == 1:
            ...
            out.write('<'+str(t3[0]) + ' ' + str(t3[1])+'> ' + str(ave_tasks[0]) + ' ' + str(ave_tasks[2]) + ' ' + str(t1) + ' ' + str(ave_tasks[1]) + ' '+ str(ave_tasks[3]) + ' ' + str(t2) + '\n')

        else:
            out.write('<'+str(t3[0]) + ' ' + str(t3[1])+'> ' + str(ave_tasks[7]) + ' ' + str(ave_tasks[8]) + ' ' + str(ave_tasks[9]) + ' ' + str(ave_tasks[10]) + '\n')

    fig = plt.figure()

    objects = ('ta', 'tda', 'tc', 'tdc', 'sta', 'stc', 'cn', 'req', 'r_s', 'r_r', 'r_ra', 'r_rs', 'd_oa', 'd_oc')
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print 'Usage: ./plot_2.py static/dynamic case_name part filename'
        sys.exit()
